...
<div class="flex justify-center gap-4 relative">
    ...
    <div class="w-full md:w-[590px]">
        ...
        <div class="stories-container">
            ...
            @foreach(['Taylor Otwell', 'Elon Musk', 'Miku Hatsune'] as $index => $name)
            <div class="story-card">
                <div class="story-overlay"></div>
                <img src="https://ui-avatars.com/api/?name={{ urlencode($name) }}" class="story-avatar">
                <img src="https://images.unsplash.com/photo-{{ $index == 0 ? '1498050108023-c5249f4df085' : ($index == 1 ? '1517694712202-14dd9538aa97' : '1550439062-609e1531270e') }}?w=200" class="story-bg">
                <div class="story-name">{{ $name }}</div>
            </div>
            @endforeach
        </div>

        <div class="card p-3 mb-4" x-data="{ content: '', hasImage: false, fileName: '' }">
            <div class="flex gap-2 border-b border-[#e9eaed] pb-3 mb-2">
                <img src="{{ Auth::user()->avatar }}" class="w-10 h-10 rounded-full border border-black/10 object-cover">
                
                <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data" class="flex-grow w-full relative">
                    @csrf
                    <div class="bg-[#f0f2f5] hover:bg-[#e4e6eb] rounded-2xl px-3 py-2 transition-colors cursor-text" 
                         @click="$refs.postInput.focus()">
                        <input x-ref="postInput"
                               type="text" 
                               name="content" 
                               x-model="content"
                               class="w-full bg-transparent border-none focus:ring-0 text-[#050505] text-[15px] placeholder-gray-600 px-0 py-0" 
                               placeholder="¿Qué estás pensando, {{ Auth::user()->first_name }}?" 
                               autocomplete="off">
                    </div>
                    <input type="file" 
                           name="image" 
            ...
            <div x-show="hasImage" class="px-2 pb-2 text-sm text-green-600 flex justify-between items-center" style="display:none" x-transition>
                <div class="flex items-center gap-2">
                    <span class="text-lg">📷</span>
                    <span x-text="fileName" class="font-semibold truncate max-w-[200px]"></span>
                </div>
                <button type="button" 
                        @click="hasImage=false; fileName=''; $refs.imageInput.value=''" 
                        class="text-gray-400 hover:text-red-500 font-bold p-1">
                    ✕
                </button>
            </div>

            <div class="flex justify-between px-2 pt-1 items-center">
                <div class="flex w-full sm:w-2/3 justify-between sm:justify-start sm:gap-2">
                    <div class="flex items-center gap-2 cursor-pointer hover:bg-gray-100 px-2 py-1 rounded justify-center flex-1 sm:flex-none">
                        <span class="text-red-500 font-bold text-xl">🎥</span>
                        <span class="text-[14px] font-semibold text-[#65676b] hidden sm:block">Video en vivo</span>
                    </div>
                    <div @click="$refs.imageInput.click()" class="flex items-center gap-2 cursor-pointer hover:bg-gray-100 px-2 py-1 rounded justify-center flex-1 sm:flex-none transition">
                        <span class="text-green-500 font-bold text-xl">📷</span>
                        <span class="text-[14px] font-semibold text-[#65676b] hidden sm:block">Foto/video</span>
                    </div>
                    <div class="flex items-center gap-2 cursor-pointer hover:bg-gray-100 px-2 py-1 rounded justify-center flex-1 sm:flex-none">
                        <span class="text-yellow-500 font-bold text-xl">😊</span>
                        <span class="text-[14px] font-semibold text-[#65676b] hidden sm:block">Sentimiento</span>
                    </div>
                </div>

                <div x-show="content.length > 0 || hasImage" style="display: none;" x-transition>
                    <button @click="$root.querySelector('form').submit()" 
                            class="bg-[#1877f2] hover:bg-[#166fe5] text-white font-bold px-6 py-1 rounded text-sm transition shadow-sm">
                        Publicar
                    </button>
                </div>
            </div>
