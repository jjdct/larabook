...
@section('title', 'Libros de ' . $user->name . ' | Larabook')
...
<style>
    ...
    .media-card { display: flex; background: white; border: 1px solid #e9ebee; border-radius: 3px; overflow: hidden; height: 120px; }
    .media-cover { width: 80px; height: 100%; object-fit: cover; }
    .media-info { padding: 10px 15px; flex-grow: 1; display: flex; flex-direction: column; justify-content: center; }
    .media-title { font-weight: bold; color: #365899; font-size: 16px; margin-bottom: 4px; display: block; }
    .media-title:hover { text-decoration: underline; }
    .media-meta { color: #90949c; font-size: 12px; }
    .media-action { padding-right: 15px; display: flex; align-items: center; }
</style>
@endpush
            ...
            <div class="max-w-[851px] mx-auto relative">
                ...
                <div class="h-[43px] bg-white flex items-center justify-between pl-[180px]">
                    <div class="flex h-full">
                        ...
                        <div class="profile-nav-item active border-b-2 border-[#385898] text-[#4b4f56]">Libros</div>
                    </div>
        ...
        <div class="max-w-[851px] mx-auto mt-4 pb-20">
            <div class="bg-white border border-[#dddfe2] rounded-sm min-h-[400px]">
                <div class="p-4 border-b border-[#e5e5e5] flex justify-between items-center">
                    <h2 class="text-[20px] font-bold text-[#1c1e21] flex items-center gap-2"><span class="text-gray-500">📚</span> Libros</h2>
                    @if(Auth::id() === $user->id) <button class="text-[#1877f2] text-sm font-semibold hover:bg-blue-50 px-3 py-1 rounded">+ Agregar libros</button> @endif
                </div>
                <div class="p-4">
                    <div class="media-grid">
                        @foreach(['Clean Code', '1984', 'Dune', 'Harry Potter'] as $book)
                        <div class="media-card">
                            <div class="media-cover bg-amber-100 flex items-center justify-center text-amber-800 text-xs text-center p-1 border-r border-amber-200">
                                {{ $book }}
                            </div>
                            <div class="media-info">
                                <a href="#" class="media-title">{{ $book }}</a>
                                <div class="media-meta">Libro · Leído</div>
                            </div>
