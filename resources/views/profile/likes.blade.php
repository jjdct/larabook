...
@section('title', 'Me gusta de ' . $user->name . ' | Larabook')
...
<style>
    ...
    /* Estilos Likes */
    .likes-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
    @media(max-width: 768px) { .likes-grid { grid-template-columns: 1fr; } }
    
    .like-card { display: flex; align-items: center; background: white; border: 1px solid #e9ebee; border-radius: 3px; overflow: hidden; }
    .like-img { width: 80px; height: 80px; object-fit: cover; }
    .like-info { padding: 0 15px; flex-grow: 1; }
    .like-title { font-weight: bold; color: #365899; font-size: 16px; display: block; }
    .like-title:hover { text-decoration: underline; }
    .like-category { color: #90949c; font-size: 12px; margin-top: 2px; }
</style>
@endpush
            ...
            <div class="max-w-[851px] mx-auto relative">
                <div class="relative w-full h-[315px] bg-gray-300 overflow-hidden group">
                    ...
                    <h1 class="absolute bottom-[20px] left-[180px] text-white text-[30px] font-bold text-shadow drop-shadow-md flex items-center z-10">
                        {{ $user->name }} @if($user->is_verified ?? false) <span class="text-[#1877f2] bg-white rounded-full w-5 h-5 inline-flex items-center justify-center text-[12px] ml-2">✓</span> @endif
                    </h1>
                    @if(Auth::id() === $user->id)
                    <form action="{{ route('profile.update_photo') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <label class="absolute top-4 left-4 bg-black/50 hover:bg-black/70 text-white p-2 rounded cursor-pointer hidden group-hover:block text-sm font-bold transition z-20">📷 Actualizar portada <input type="file" name="image" class="hidden-input" accept="image/*" onchange="this.form.submit()"></label>
                    </form>
                    @endif
                </div>
                <div class="h-[43px] bg-white flex items-center justify-between pl-[180px]">
                    <div class="flex h-full">
                        ...
                        <div class="profile-nav-item active border-b-2 border-[#385898] text-[#4b4f56]">Me gusta</div>
                    </div>
                ...
                <div class="absolute top-[160px] left-[15px] p-1 bg-white rounded-full border border-[rgba(0,0,0,.1)] z-10 group">
                    ...
                    <form action="{{ route('profile.update_photo') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <label class="absolute bottom-2 left-0 right-0 text-center bg-black/60 text-white text-xs py-1 cursor-pointer opacity-0 group-hover:opacity-100 transition z-20 rounded-b-full mx-1">📷 Actualizar <input type="file" name="image" class="hidden-input" accept="image/*" onchange="this.form.submit()"></label>
                    </form>
                    @endif
                </div>
            </div>
        ...
        <div class="max-w-[851px] mx-auto mt-4 pb-20">
            <div class="bg-white border border-[#dddfe2] rounded-sm min-h-[400px]">
                <div class="p-4 border-b border-[#e5e5e5] flex justify-between items-center">
                    <h2 class="text-[20px] font-bold text-[#1c1e21] flex items-center gap-2"><span class="text-gray-500">👍</span> Me gusta</h2>
                </div>
                <div class="p-4">
                    <div class="likes-grid">
                        @foreach(['Laravel News', 'GitHub', 'SpaceX', 'Nintendo'] as $page)
                        <div class="like-card">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($page) }}&background=random&size=100" class="like-img">
                            <div class="like-info">
                                <a href="#" class="like-title">{{ $page }}</a>
                                <div class="like-category">Tecnología · {{ rand(100, 900) }} mil seguidores</div>
                            </div>
                            <div class="pr-4">
                                <button class="bg-[#f5f6f7] hover:bg-[#e9ebee] border border-[#ccd0d5] text-[#4b4f56] font-bold text-xs px-3 py-1.5 rounded">
                                    ✓ Te gusta
                                </button>
                            </div>
                        </div>
