...
<head>
    ...
    <style>
        ...
        .msg-row { display: flex; gap: 10px; margin-bottom: 10px; padding: 5px 15px; }
        .msg-row:hover { background-color: #f6f7f9; }
        /* Solo aplicamos estilo 'propio' si realmente es el usuario humano */
        .msg-own { background-color: #f6f7f9; border-top: 1px solid #e9eaed; border-bottom: 1px solid #e9eaed; }

        .btn-reply { background-color: #4e69a2; border: 1px solid #435a8b; color: white; font-weight: bold; padding: 4px 12px; border-radius: 2px; cursor: pointer; }
        .btn-new-msg { background: linear-gradient(#f6f7f9, #ebedf0); border: 1px solid #ced0d4; color: #4b4f56; font-weight: bold; padding: 4px 8px; border-radius: 2px; cursor: pointer; text-decoration: none; }
    </style>
</head>
<body>
    ...
    <nav class="fb-dark-blue fixed top-0 w-full z-50 h-[42px] border-b border-[#29487d] flex items-center justify-between px-4 md:px-20 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div class="hidden md:flex bg-white rounded-[3px] border border-[#203a6d] h-6 items-center w-[350px]">
                ...
                <button class="bg-[#f2f2f2] h-full px-2 flex items-center border-l border-gray-300 cursor-pointer">🔍</button>
            </div>
        </div>
            ...
            <div class="inbox-sidebar">
                ...
                <div class="overflow-y-auto flex-1">
                    @forelse($conversations as $conv)
                        @php
                        // 🛡️ PROTECCIÓN CONTRA FANTASMAS
                        // Si por alguna razón el usuario es null (ej. usuario borrado), saltamos esta vuelta.
                        if (!$conv['user']) {
                            continue; 
                        }

                            // Verificamos si este es el chat activo
                        ...
                        <a href="{{ route('messages.show', $conv['user']) }}" class="thread-item {{ $isActive ? 'active' : '' }}">
                            ...
                            <div class="flex-1 overflow-hidden">
                                ...
                                <div class="text-[11px] text-gray-500 truncate">
                                    @if($conv['last_message']->sender_id === auth()->id() && $conv['last_message']->sender_type === 'App\Models\User')
                ...
                @if(isset($user))
                    <div class="msg-header shrink-0">
                        ...
                        <div class="flex gap-3 text-gray-500 font-bold text-[12px] items-center">
                            @if((isset($isArchivedView) && $isArchivedView) || (isset($isCurrentChatArchived) && $isCurrentChatArchived))
                                <form action="{{ route('messages.unarchive', $user) }}" method="POST" class="m-0">
                                    @csrf
                    ...
                    <div class="flex-1 overflow-y-auto p-0 bg-white" id="messages-container">
                        @foreach($chat as $message)
                            @php
                                // AQUÍ ARREGLAMOS LA CRISIS DE IDENTIDAD
                                // Solo es "Mío" si el ID coincide Y el tipo es User
                                $isMe = $message->sender_id === auth()->id() && $message->sender_type === 'App\Models\User';
                            @endphp

                            <div class="msg-row {{ $isMe ? 'msg-own' : '' }}">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($message->sender->name ?? 'Usuario') }}&background=random" class="w-10 h-10 border border-gray-300 shrink-0">
                                <div class="max-w-[80%]">
                                    <div class="text-[13px] font-bold text-[#3b5998] cursor-pointer hover:underline mb-0.5">
                                        {{ $isMe ? 'Tú' : ($message->sender->name ?? 'Usuario') }}
                                    </div>
                    ...
                    <div class="bg-[#f2f2f2] border-t border-[#bdc7d8] p-3 shrink-0">
                        <form action="{{ route('messages.store', $user) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="flex justify-between items-center">
                                <div class="flex gap-3 text-[11px] text-[#3b5998]">
                                    <label class="flex items-center gap-1 cursor-pointer hover:underline">
                                        ...
                                        <span class="font-bold">📷 Añadir fotos/archivos</span>
                                    </label>
                                </div>
